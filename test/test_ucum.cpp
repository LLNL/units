    ...
    for (const auto &junit : defs["units"])
    {
        ...
        if (!(ciCode.empty() || csCode.empty()))
        {
            if (ciact != csact)
            {
                ...
                std::cout << ciCode << " and " << csCode << " do not produce the same unit " << junit["name_"]
                          << '\n';
                ++ceqFail;
            }
        }
...
TEST(UCUM, TestMatchingPrint)
{
    ...
    for (const auto &junit : defs["units"])
    {
        ...
        if (junit.contains("printSymbol_"))
        {
            ...
            if (name == "a" || name == "K" || name == "oz fl" || name == "btu" || name == "S")
            ...
            if (name == "X" || name == "C" || name == "M" || name == "Q" || name == "B" || name == "%")
            {  // known conflicts are and Kayser with annum and Kelvin
               // oz fl has multiple meanings
               // btu default is different for power dominant vs thermal dominant
                continue;
            }
            if (csCode.front() == '[' && csCode.back() == ']' && name.find_first_of('.') != std::string::npos)
            {
                continue;
            }
...
TEST(UCUM, TestRoundTrip)
{
    ...
    for (const auto &junit : defs["units"])
    {
        ...
        if (uact != csact)
        {
            std::cout << ustring << " and " << csCode << " do not produce the same unit " << junit["name_"]
                      << '\n';
            ++ceqFail;
        }
    }
...
TEST(UCUMConversions, Interpret1)
{
    ...
    auto cs = doc.FirstChildElement("ucumTests")->FirstChildElement("validation")->FirstChildElement("case");
    int failConvert = 0;
    while (cs)
    {
        ...
        if (prodUnit.is_error())
        {
            if (valid)
            {
                std::cout << "unable to convert " << unit << " into a valid unit : unit test id " << id << '\n';
                ++failConvert;
            }
        }
        else if (!valid)
...
TEST(UCUMConversions, convert1)
{
    ...
    auto cs = doc.FirstChildElement("ucumTests")->FirstChildElement("conversion")->FirstChildElement("case");
    int failConvert = 0;
    while (cs)
    {
