...
struct convValue
{
    ...
    double value{0};
    convValue() = default;
    convValue(const std::string &nm, const std::string &sn, double val) : name(nm), short_name(sn), value(val) {}
};
...
TEST_P(converterApp, fileConversions)
{
    ...
    for (auto &convcode : unit_conv)
    {
        ...
        EXPECT_NEAR(res / convcode.value, 1.0, 0.003)
          << unit_conv[0].name << " to " << convcode.name << " produced " << res << " when " << convcode.value
          << " expected";
