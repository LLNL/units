...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size)
{
    ...
    if (!units::is_valid(meas1)) {
        ...
        bool match = false;
        if (units::measurement_cast(meas2) == units::measurement_cast(meas1)) {
            match = true;
        } else if (isnormal(root(meas2, 2))) {
            if (root(units::measurement_cast(meas2), 2) == root(units::measurement_cast(meas1), 2)) {
                match = true;
            }
        }
        if (!match) {
            if (isnormal(root(meas2, 3))) {
                if (root(units::measurement_cast(meas2), 3) == root(units::measurement_cast(meas1), 3)) {
                    match = true;
                }
            }
        }
