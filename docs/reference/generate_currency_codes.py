...
for index, row in xls_data.iterrows(): 
    if index >= 3: 
        ...
        line1=f'{{"{currency}",{{1.0, precise::currency, generateCurrencyCode("{row.iloc[2]}")}}}},'
        outfile.write(f'{line1}\n')

outfile.write('\n\n\n')
outfile.write('UNITS_CPP14_CONSTEXPR_OBJECT std::array<std::pair<const char*, std::uint32_t>, 1213> defined_commodity_codes{\n')
